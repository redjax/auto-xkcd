...
class CelerySettings(BaseModel):
    broker_host: str = Field(
        default=CELERY_SETTINGS.get("CELERY_BROKER_HOST", default="localhost")
    )
    broker_port: t.Union[int, str] = Field(
        default=CELERY_SETTINGS.get("CELERY_BROKER_PORT", default=5672)
    )
    broker_username: str = Field(
        default=CELERY_SETTINGS.get("CELERY_BROKER_USERNAME", default="guest")
    )
    broker_password: str | None = Field(
        default=CELERY_SETTINGS.get("CELERY_BROKER_PASSWORD", default="")
    )
    broker_vhost: str = Field(
        default=CELERY_SETTINGS.get("CELERY_BROKER_VHOST", default="/")
    )
    backend_host: str | None = Field(
        default=CELERY_SETTINGS.get("CELERY_BACKEND_HOST", default="localhost")
    )
    backend_port: t.Union[int, str] = Field(
        default=CELERY_SETTINGS.get("CELERY_BACKEND_PORT", default=6379)
    )
    ...
    def broker_url(self) -> str:
        if self.broker_password:
            broker_url: str = (
                f"amqp://{self.broker_username}:{self.broker_password}@{self.broker_host}:{self.broker_port}"
            )
        else:
            broker_url: str = (
                f"amqp://{self.broker_username}:''@{self.broker_host}:{self.broker_port}"
            )
