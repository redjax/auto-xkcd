...
def demo_current_comic():
    xkcd_api_controller: xkcdapi.controllers.XkcdApiController = (
        xkcdapi.controllers.XkcdApiController(cache_ttl=DEMO_CACHE_TTL)
    )
...
def main(db_engine: sa.Engine):
    ...
    db_current_comic, db_current_comic_img = xkcdapi.db_client.save_comic_and_img_to_db(
        comic=current_comic, comic_img=current_comic_img, engine=db_engine
    )
    log.debug(
        f"Saved current comic: {db_current_comic} and curret comic image: {db_current_comic_img}"
    )
    ...
    comics, comic_imgs = demo_multiple_comics(
        num_rand_comics=3, current_comic_num=current_comic.num
    )
    ...
    if (not comics) or (isinstance(comics, list) and len(comics) == 0):
        log.warning(
            "List of random comics is empty, there may have been an error requesting multiple comics."
        )
    else:
    ...
    if (not comic_imgs) or (isinstance(comic_imgs, list) and len(comic_imgs) == 0):
        log.warning(
            "List of random comic images is empty, there may have been an error requesting multiple comics."
        )
    else:
        log.success(f"Requested [{len(comic_imgs)}] random comic image(s)")

    db_comics, db_comic_imgs = xkcdapi.db_client.save_multiple_comics_and_imgs_to_db(
        comics=comics, comic_imgs=comic_imgs, engine=db_engine
    )
