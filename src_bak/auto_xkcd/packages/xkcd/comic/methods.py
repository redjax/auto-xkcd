...
def get_comic(
    ...
    transport: t.Union[httpx.HTTPTransport | hishel.CacheTransport] = None,
) -> httpx.Response:
    ...
    assert transport, ValueError("Missing request Transport")
    assert isinstance(transport, httpx.HTTPTransport) or isinstance(
        transport, hishel.CacheTransport
    ), TypeError(
        f"Expected transport to be of type httpx.HTTPTransport or hishel.CacheTransport. Got type: ({type(transport)})"
    )
